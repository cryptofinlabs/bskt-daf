...
const NATURAL_UNIT = new BigNumber('1e18');
...
contract('RebalancingBsktToken', function(accounts) {
  ...
  async function setupRebalancingBsktToken(
    ...
    state.feeAmount = state.feeAmount;
    ...
    if (isFee) {
      await state.feeToken.mint(state.user1, NATURAL_UNIT.times(100), { from: state.owner });
      await state.feeToken.approve(state.rebalancingBsktToken.address, NATURAL_UNIT.times(100), { from: state.user1 });
      await state.feeToken.approve(state.escrow, NATURAL_UNIT.times(100), { from: state.user1 });
    }
    for (let i = 0; i < state.tokens.length; i++) {
      await state.tokens[i].mint(state.user1, NATURAL_UNIT.times(100), { from: state.owner });
      await state.tokens[i].approve(state.rebalancingBsktToken.address, NATURAL_UNIT.times(100), { from: state.user1 });
      await state.tokens[i].approve(state.escrow, NATURAL_UNIT.times(100), { from: state.user1 });
    }
    return state;
  }
  ...
  context('with fresh RebalancingBsktToken and zero fees', function() {
    ...
    it('should compare bids correctly', async function() {
      ...
      await state.bsktRegistry.set(3, state.tokens[3].address, 100, { from: state.dataManager });
      await state.bsktRegistry.set(4, state.tokens[4].address, 100, { from: state.dataManager });
      await setupRegistryStateB(state.dataManager, state.bsktRegistry, state.tokens[0], state.tokens[1], state.tokens[2], state.tokens[3], state.tokens[4])
  ...
  context('with simple initial allocation and zero fees', function() {
    ...
    it('should bid successfully for first bid', async function() {
      ...
      assert.equal(user1BalanceA, NATURAL_UNIT.times(100), 'user1 state.tokens[0] balance should be unchanged');
      assert.equal(user1BalanceB, NATURAL_UNIT.times(100), 'user1 state.tokens[1] balance should be unchanged');
      assert.equal(user1BalanceC, NATURAL_UNIT.times(100) - 150, 'user1 state.tokens[1] balance should be 150 less');
    ...
    it('should bid and rebalance correctly for perfect bid', async function() {
      ...
      let totalUnitsStart = await state.rebalancingBsktToken.totalUnits.call();
      let creationSizeStart = await state.rebalancingBsktToken.creationSize.call();
      console.log('totalUnitsStart', totalUnitsStart.toNumber());
      console.log('creationSizeStart', creationSizeStart.toNumber());
      ...
      let totalSupply = await state.rebalancingBsktToken.totalSupply.call();
      console.log('totalSupply', totalSupply.toNumber());
      let totalUnits = await state.rebalancingBsktToken.computeTotalUnits.call(creationSize, [50, 150, 0]);
      console.log('totalUnits', totalUnits.toNumber());
      let computedCreationSize = await state.rebalancingBsktToken.computeCreationSize.call([50, 150, 0]);
      console.log('computedCreationSize', computedCreationSize.toNumber());

      await state.rebalancingBsktToken.rebalance({ from: state.user1 });

      let totalUnitsEnd = await state.rebalancingBsktToken.totalUnits.call();
      let creationSizeEnd = await state.rebalancingBsktToken.creationSize.call();
      console.log('totalUnitsStart', totalUnitsStart.toNumber());
      console.log('creationSizeEnd', creationSizeEnd.toNumber());
      ...
      //const [updatedTokens, updatedQuantities] = await state.rebalancingBsktToken.getCreationUnit.call();
  ...
  context('with initial allocation and zero fees', function() {
    ...
    it('should get rebalance deltas', async function() {
      await setupRegistryStateA(dataManager, state.bsktRegistry, state.tokens[0], state.tokens[1], state.tokens[2], state.tokens[3], state.tokens[4]);
      let [targetTokens, deltas] = await state.rebalancingBsktToken.getRebalanceDeltas.call();
  ...
  context('helper functions', function() {
    ...
    it('should compute total units correctly', async function() {
      const totalSupply = NATURAL_UNIT;
      const quantities = [0, 10, 15];
      const totalUnits = await state.rebalancingBsktToken.computeTotalUnits.call(totalSupply, quantities);
      console.log('totalUnits', totalUnits.toNumber());
      assert.equal(totalUnits.toNumber(), 10);
    });
